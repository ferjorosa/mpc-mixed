...
public class EF_Dirichlet extends EF_UnivariateDistribution {
     ...
     * @param parameters
     */
    // Le pasamos los moment parameters, aplicamos la formula de https://en.wikipedia.org/wiki/Exponential_family
    // En la dirichlet da igual porque son equivalentes, pero en otras distribuciones influye
    public EF_Dirichlet(Variable var, double[] parameters) {
        if (!var.isDirichletParameter())
            throw new IllegalArgumentException("The variable is not a Dirichlet parameter!");
        if(var.getNumberOfStates() != parameters.length)
            throw new IllegalArgumentException("There is a discrepancy in the number of prior parameters and the number of parameters in the distribution");
    ...
    */
    ...
    public AmidstVector getExpectedParameters() {
        ...
        AmidstVector vector = new ArrayVector(this.nOfStates);
        for (int i = 0; i < nOfStates; i++) {
