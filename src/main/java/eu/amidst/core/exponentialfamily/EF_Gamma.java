...
public class EF_Gamma extends EF_UnivariateDistribution {
    ...
    /* MyNote: finalmente no lo he utilizado, tengo varias incognitas en Normal_Normal_pParents que es donde
    *  se utilizaria este constructor (GammaParameter). No entiendo los momentos, quizas habria que mirar mas wikipedia,
    *  aunque no he encontrado nada (de la forma que si habia encontrado algo sobre la Normal-Gamma)
    * */
    // Le pasamos los parameters (no son los moment...)
    public EF_Gamma(Variable var, double[] parameters) {
        if (!var.isGammaParameter())
            throw new IllegalArgumentException("The variable is not a Gamma parameter!");
        if(parameters.length != 2)
            throw new IllegalArgumentException("A Gamma distribution has 2 parameters. You have passed "+ parameters.length);
    ...
    public AmidstVector getExpectedParameters() {
        ...
        AmidstVector vec = new ArrayVector(1);
