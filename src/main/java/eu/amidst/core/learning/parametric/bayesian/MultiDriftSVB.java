...
public class MultiDriftSVB extends SVB {
    ...
    public void initLearning() {
        ...
        prior = this.plateuStructure.getParameterVariablePriors();
    ...
    public BayesianNetwork getLearntBayesianNetwork() {
        ...
        CompoundVector prior = this.plateuStructure.getParameterVariablePriors();

        this.updateNaturalParameterPrior(this.plateuStructure.getParameterVariablesPosterior());
    ...
    public double updateModelWithConceptDrift(DataOnMemory<DataInstance> batch) {
        ...
        if (firstBatch){
            ...
            posteriorT_1 = this.plateuStructure.getParameterVariablesPosterior();
            this.plateuStructure.updateParameterVariablesPrior(posteriorT_1);
        ...
        while(!convergence && niter<10) {
            ...
            this.plateuStructure.updateParameterVariablesPrior(newPrior);
            ...
            int count = 0;
            //Messages to TExp
            this.plateuStructure.updateParameterVariablesPrior(this.prior);
            for (Node node : this.plateuStructure.getNonReplicatedNodes().collect(Collectors.toList())) {
            ...
            count = 0;
            //Messages to TExp
            this.plateuStructure.updateParameterVariablesPrior(this.posteriorT_1);
            for (Node node : this.plateuStructure.getNonReplicatedNodes().collect(Collectors.toList())) {
                Map<Variable, MomentParameters> momentParents = node.getMomentParents();
