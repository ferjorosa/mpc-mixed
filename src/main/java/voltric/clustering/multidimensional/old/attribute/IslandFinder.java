...
import voltric.clustering.multidimensional.old.attribute.stopcondition.StopCondition;
...
public class IslandFinder implements AttributeGrouping{
    ...
    public List<HLCM> find(DiscreteData dataSet){
        ...
        while(variableSet.size() > 0){
            ...
            if(variableSet.size() == 3){
                ...
                HLCM lcm = HlcmCreator.createLCM(variableSet, 2, new Random());
                islands.add(lcm);
                return islands;
    ...
    private Tuple<DiscreteVariable, DiscreteVariable> findBestPair(List<DiscreteVariable> variableSet, Map<DiscreteVariable, Map<DiscreteVariable, Double>> pairScores){
