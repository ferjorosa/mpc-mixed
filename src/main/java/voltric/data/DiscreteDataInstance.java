...
public class DiscreteDataInstance {
     ...
     */
    void setData(DiscreteData data){
    ...
    DiscreteDataInstance project(DiscreteData data){

        List<DiscreteVariable> dataVariables = data.getVariables();

        if(this.getVariables().equals(dataVariables))
            return this;

        int[] projectedValues = new int[data.getVariables().size()];

        for(DiscreteVariable variable: dataVariables)
            projectedValues[dataVariables.indexOf(variable)] = this.getNumericValue(variable);

        DiscreteDataInstance projectedDataInstance = new DiscreteDataInstance(projectedValues);
        projectedDataInstance.setData(data);

        return projectedDataInstance;
    }
