...
public abstract class AbstractGraph<T> implements Cloneable {
	 ...
	 * @return {@code true} if the edge is present and {@code false} otherwise
	 */
	public abstract boolean containsEdge(AbstractNode<T> head, AbstractNode<T> tail);

	/**
	 * Checks if the edge is allowed
	 *
	 * @param head the head node.
	 * @param tail the tail node.
	 * @return {@code true} if the edge is allowed and {@code false} otherwise
	 */
	public abstract boolean isEdgeAllowed(AbstractNode<T> head, AbstractNode<T> tail);

	//TODO: docs
	public abstract void dfs(AbstractNode<T> node, Map<AbstractNode<T>, Integer> visitedNodes);
	...
	public final boolean containsPath(AbstractNode<T> start, AbstractNode<T> end) {
		...
		LinkedHashMap<AbstractNode<T>, Integer> visitedNodes = new LinkedHashMap<>();

		// DFS
		dfs(start, visitedNodes);

		// returns true if the end has been discovered
		return visitedNodes.containsKey(end);
	}
