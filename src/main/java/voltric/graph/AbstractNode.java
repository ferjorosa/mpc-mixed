...
public abstract class AbstractNode<T> {
	 ...
	 * TODO: Previamente era un Map<AbstractNode<T>, Edge<T>>, pero daba problemas al ser la Key mutable cuando eran BeliefNodes
	 */
	protected LinkedHashMap<T, Edge<T>> neighbors;
	...
	protected void attachEdge(Edge<T> edge) {
		...
		neighbors.put(edge.getOpposite(this).getContent(), edge);
	}
	...
	protected void detachEdge(Edge<T> edge) {
		...
		neighbors.remove(edge.getOpposite(this).getContent());
	}
	...
	public Edge<T> getEdge(AbstractNode<T> node) {
		...
		return neighbors.get(node.getContent());
	}
	...
	public final Set<AbstractNode<T>> getNeighbors() {
		// TODO: He modificado el codigo por ultima vez cuando Map de neighbors cambio el tipo de KeySet
		return neighbors.keySet().stream().map(x-> graph.getNode(x)).collect(Collectors.toCollection(LinkedHashSet::new));
	}
	...
	public final boolean hasNeighbor(AbstractNode<T> node) {
		...
		return neighbors.containsKey(node.getContent());
	}
