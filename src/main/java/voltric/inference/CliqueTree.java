...
//		buildCliqueTree(moralGraph, order.iterator(), DiscreteBayesNet);
		...
		for (DiscreteVariable var : DiscreteBayesNet.getVariables()) {
			...
			for (AbstractNode clique : this.nodes) {
				...
				if (card < minCard && cliqueNode.contains(var) && cliqueNode.containsAll(vParents)) {
					minCard = card;
	...
	// TODO: Ojo, como el nodo no pertenezca al tree, hay lio asegurado.
	public void setPivot(CliqueNode node) { this._pivot = node; }
	 ...
	 *
	 * Note: It may cause a StackOverflow error if the tree is large
	 * 
	 * @param moralGraph
