...
public class Function implements Cloneable, Potential {
    ...
    public final static Function createFunction(DiscreteData dataSet) {
        ...
        for (DiscreteDataInstance dataCase : dataSet.getInstances()) {
            index = f.computeIndex(dataCase.getNumericValues());
            f._cells[index] = dataSet.getWeight(dataCase);
        }
     ...
     *         arguments.
     */
    public Function project(List<DiscreteVariable> vars, List<Integer> states) {
    ...
    /**
     * Makes this function a collection of random distributions of the specified
     * variable. It has a specific seed.
     */
    public final void randomlyDistribute(DiscreteVariable variable, Random random) {
        // randomly sets cells within (0.0, 1.0)
        // I try to avoid likelihood = 0 by excluding zero.
        int domainSize = getDomainSize();
        for (int i = 0; i < domainSize; i++)
    ...
    /* 27-09-2018 codigo creado para CombineBNs */
    public static Function createFullyDefinedFunction(List<DiscreteVariable> variables, double[] cells, int[] magnitudes) {
        ...
        // creates specialized function
        switch (varArray.length) {

            case 1:
                f = new Function1D(varArray, cells, magnitudes);
                break;

            case 2:
                f = new Function2D(varArray, cells, magnitudes);
                break;

            default:
                f = new Function(varArray, cells, magnitudes);
                break;
        }
